...
def main():
    ...
    parser.add_argument('--cameras', help='cameras.txt file', default='colmap_data/sparse/0/converter/cameras.txt', required=False)
    parser.add_argument('--points', help='points3D.txt file', default='colmap_data/sparse/0/converter/points3D.txt', required=False)
    parser.add_argument('--images', help='images.txt file', default='colmap_data/sparse/0/converter/images.txt', required=False)
    parser.add_argument('--input', help='input image directory', default='preprocessed/undistorted_scene', required=False)
    parser.add_argument('--output', help='reprojection output directory', default='preprocessed/reprojected_undistorted_scene', required=False)
    args = parser.parse_args()

    K = read_camera(args.cameras)
    points3D, color = read_points(args.points)
    Rt, keypoints2d = read_images(args.images)
    reproject(K,Rt,points3D, color, keypoints2d, args.input, args.output)
